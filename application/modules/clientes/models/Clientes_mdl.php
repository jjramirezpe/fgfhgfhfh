...
class Clientes_mdl extends CI_Model {
	...
	var $order = array('id_cliente' => 'desc'); // default order 
	...
	{
		$this->db->insert('notas_cliente', $data);
		return $this->db->insert_id();
	}
	...
	/*funcion para ver el balance de un cliente*/
	public function get_balance($id)
	{
		$this->db->select_sum('total');
		$this->db->where('id_cliente',$id);
		$this->db->where('estado', 0);
		$query = $this->db->get('lista_facturas');
		return $query->row();
	}


	/*total facturado a un cliente*/
	public function get_total_facturado($id)
	{
		$this->db->select_sum('total');
		$this->db->where('id_cliente',$id);
		$query = $this->db->get('lista_facturas');
	...
	/*facturas vigentes*/
	public function get_vigentes($id)
	{
		$this->db->select('id_factura, numero_factura, fecha_factura, fecha_vencimiento, total');
		$this->db->where('id_cliente',$id);
		$this->db->where('estado', 0);
		$query = $this->db->get('lista_facturas');
		return $query->result();
